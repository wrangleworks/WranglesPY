...
class TestMergeCoalesce:
    ...
    def test_coalesce_empty_dataframe(self):
        ...
        """
        data = pd.DataFrame({"Col1": [], "Col2": [], "Col3": []})
        recipe = """
        wrangles:
        - merge.coalesce:
            input:
...
class TestMergeConcatenate:
    ...
    def test_concatenate_strings(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["Output Col"] == "AAAAAA"
            and df.iloc[2]["Output Col"] == "HHHHHH"
        )
...
class TestMergeLists:
    ...
    def test_lists_1(self):
        ...
        """
        data = pd.DataFrame({"Col1": [["A", "B"]], "Col2": [["D", "E"]]})
        recipe = """
        wrangles:
            - merge.lists:
                input: 
    ...
    def test_lists_where(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df.iloc[0]["Combined Col"] == "" and df.iloc[1]["Combined Col"] == [
            "C",
            "D",
            "F",
            "G",
    ...
    def test_lists_remove_duplicates(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df.iloc[0]["Combined Col"] == ["A", "B", "C", "D", "E"] and df.iloc[1][
            "Combined Col"
        ] == ["F", "G", "H", "I"]
    ...
    def test_lists_remove_duplicates_ignore_case_false(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df.iloc[0]["Combined Col"] == ["A", "B", "c", "D", "E", "C"] and df.iloc[
            1
        ]["Combined Col"] == ["f", "G", "H", "F", "g", "I"]
    ...
    def test_lists_ignore_case_false(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df.iloc[0]["Combined Col"] == ["A", "B", "D", "B"] and df.iloc[1][
            "Combined Col"
        ] == ["f", "G", "F", "g"]
...
class TestMergeToDict:
    ...
    def test_to_dict_where(self):
        ...
        """
        data = pd.DataFrame(
            {"Col1": ["A", "C", "E"], "Col2": ["B", "D", "F"], "numbers": [43, 22, 65]}
        )
        recipe = """
        wrangles:
            - merge.to_dict:
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["Dict Col"] == {"Col1": "A", "Col2": "B"}
            and df.iloc[2]["Dict Col"] == ""
        )
...
class TestMergeKeyValuePairs:
    ...
    def test_wildcard_where(self):
        ...
        assert df["Object"][0] == "" and df["Object"][2] == {"C": "c", "three": "Third"}
    ...
    def test_key_value_pairs_3(self):
        ...
        """
        data = pd.DataFrame(
            {"key 1": ["A"], "key 2": [True], "value 1": ["a"], "value 2": [False]}
        )
        recipe = """
        wrangles:
            - merge.key_value_pairs:
    ...
    def test_key_value_pairs_empty_dataframe(self):
        ...
        """
        data = pd.DataFrame({"key1": [], "key2": [], "value1": [], "value2": []})
        recipe = """
        wrangles:
            - merge.key_value_pairs:
                input:
...
class TestMergeDictionaries:
    ...
    def test_merge_dicts_where(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[1]["out"] == {"Hello": "Moto", "Hola": "Hello"}
            and df.iloc[0]["out"] == ""
        )
