...
class TestConvertCase:
    ...
    def test_default(self):
        ...
        """
        data = pd.DataFrame(
            [["A StRiNg", "Another String"]], columns=["Data1", "Data2"]
        )
        recipe = """
        wrangles:
        - convert.case:
    ...
    def test_list(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["Data1"] == "a string"
            and df.iloc[0]["Data2"] == "another string"
        )
    ...
    def test_list_to_list(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["Column1"] == "a string"
            and df.iloc[0]["Column2"] == "another string"
        )
    ...
    def test_sentence(self):
        ...
        data = pd.DataFrame(
            {
                "Data": [
                    "will this be capitalized correctly? i'm not so sure! probably not... but maybe",
                    "i LOST! mY 13.5mm WRENCH.",
                ]
            }
        )
        recipe = """
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["output data"]
            == "Will this be capitalized correctly? I'm not so sure! Probably not... But maybe"
            and df.iloc[1]["output data"] == "I lost! My 13.5mm wrench."
        )
    ...
    def test_list_sentence(self):
        ...
        """
        data = pd.DataFrame(
            {"Data": [["a StRiNg.", " YET another StRiNg.", "   a THIRD STring."]]}
        )
        recipe = """
        wrangles:
        - convert.case:
    ...
    def test_where(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["Col1"] == "Ball Bearing"
            and df.iloc[2]["Col1"] == "needle bearing"
        )
...
class TestConvertDataType:
    ...
    def test_list_to_list(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert isinstance(df.iloc[0]["out1"], str) and isinstance(
            df.iloc[0]["out2"], str
        )
    ...
    def test_where(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.data_type:
                ...
                where: number > 25
            """,
            dataframe=pd.DataFrame({"number": [25, 31, 22]}),
        )
    ...
    def test_convert_to_datetime_where(self):
        ...
        """
        data = pd.DataFrame({"date": ["12/25/2050", "11/10/1987"], "numbers": [4, 2]})
        recipe = """
        wrangles:
        - convert.data_type:
            input: date
    ...
    def test_convert_datatype_invalid_int(self):
        ...
        """
        df = pd.DataFrame({"col": ["1", "2", 3, "4", 5, "", "this is text"]})
        df = wrangles.recipe.run(
            """
            wrangles:
            - convert.data_type:
    ...
    def test_convert_datatype_invalid_date(self):
        ...
        where some values cannot be converted
        """
        df = pd.DataFrame({"col": ["4/9/2222", "Look, a string!"]})
        df = wrangles.recipe.run(
            """
            wrangles:
            - convert.data_type:
        ...
        assert (
            df["output"][0] == pd.Timestamp("2222-04-09 00:00:00")
            and df["output"][1] == "Look, a string!"
        )
    ...
    def test_convert_datatype_invalid_date_default(self):
        ...
        assert df["output"][0] == pd.Timestamp("2222-04-09 00:00:00") and df["output"][
            1
        ] == pd.Timestamp("1999-08-29 00:00:00")
    ...
    def test_convert_datatype_error_invalid_data_type(self):
        ...
        assert (
            ...
            and "convert.data_type - data_type squirrel is not supported."
            in info.value.args[0]
        )
...
class TestConvertFractionToDecimal:
    ...
    def test_default(self):
        ...
        data = pd.DataFrame(
            {
                "col1": [
                    "The length is 1/2 wide 1/3 high",
                    "the panel is 3/4 inches",
                    "the diameter is 1/3 meters",
                ],
            }
        )
        recipe = """
    ...
    def test_list_to_list(self):
        ...
        data = pd.DataFrame(
            {
                ...
                "col2": [
                    "The length is 3/4 wide 1/8 high",
                    "the panel is 1/3 inches",
                    "the diameter is 3/16 meters",
                ],
            }
        )
        recipe = """
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["out1"] == "The length is 0.5 wide 0.3333 high"
            and df.iloc[0]["out2"] == "The length is 0.75 wide 0.125 high"
        )
    ...
    def test_where(self):
        ...
        assert (
            df.iloc[2]["out1"] == ""
            and df.iloc[0]["out1"] == "The length is 0.5 wide 0.3333 high"
        )
...
class TestConvertFromJSON:
    ...
    def test_array_list_to_list(self):
        ...
        """
        data = pd.DataFrame(
            [['["val1", "val2"]', '["val3", "val4"]']], columns=["header1", "header2"]
        )
        recipe = """
        wrangles:
            - convert.from_json:
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert isinstance(df.iloc[0]["out1"], list) and isinstance(
            df.iloc[0]["out2"], list
        )
    ...
    def test_array_where(self):
        ...
        """
        data = pd.DataFrame(
            {"header1": ['["val1", "val2"]', '["val3", "val4"]'], "numbers": [5, 7]}
        )
        recipe = """
        wrangles:
            - convert.from_json:
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df.iloc[0]["header1"] == '["val1", "val2"]' and isinstance(
            df.iloc[1]["header1"], list
        )
    ...
    def test_default_array(self):
        ...
        assert df["header1"][0] == [] and df["header1"][1] == [1, 2, 3]
    ...
    def test_default_dict_error(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.from_json:
                ...
                default: {}
            """,
            dataframe=pd.DataFrame({"header1": ['{"a":"b":"c"}', "[1,2,3]"]}),
        )
        assert df["header1"][0] == {} and df["header1"][1] == [1, 2, 3]
    ...
    def test_empty(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.from_json:
                ...
                output: output column
            """,
            dataframe=pd.DataFrame({"header1": []}),
        )
...
class TestConvertFromYAML:
    ...
    def test_default(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.from_yaml:
                input: column
            """,
            dataframe=pd.DataFrame({"column": ["key: val\nkey2:\n- list1\n- list2\n"]}),
        )
        assert _json.dumps(df["column"][0]) == _json.dumps(
    ...
    def test_empty_default_list(self):
        ...
        assert df["column"][0] == [] and df["column"][1]["key"] == "val"
    ...
    def test_where(self):
        ...
        assert df["output col"][0] == "" and df["output col"][1]["key3"] == "val"
...
class TestConvertToJSON:
    ...
    def test_array_list_to_list(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["out1"] == '["val1", "val2"]'
            and df.iloc[0]["out2"] == '["val3", "val4"]'
        )
    ...
    def test_array_where(self):
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["headers_json"] == ""
            and df.iloc[1]["headers_json"] == '["val2", "val5"]'
        )
    ...
    def test_array_sort_keys(self):
        ...
        """
        data = pd.DataFrame(
            [["val3", "val1", "val2"]], columns=["header3", "header1", "header2"]
        )
        recipe = """
        wrangles:
            - merge.to_dict:
    ...
    def test_numpy_array(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.to_json:
                input: column
            """,
            dataframe=pd.DataFrame({"column": [_np.array([1, "a"], dtype=object)]}),
        )
    ...
    def test_datetime(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.to_json:
                input: column
            """,
            dataframe=pd.DataFrame({"column": [pd.Timestamp("2020-01-01")]}),
        )
    ...
    def test_numpy_float(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.to_json:
                input: column
            """,
            dataframe=pd.DataFrame({"column": [[_np.float32(1)]]}, dtype=object),
        )
    ...
    def test_numpy_int(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.to_json:
                input: column
            """,
            dataframe=pd.DataFrame({"column": [[_np.int32(1)]]}, dtype=object),
        )
    ...
    def test_unicode_characters(self):
        ...
        df = wrangles.recipe.run(
            ...
            - convert.to_yaml:
                input: column
            """,
            dataframe=pd.DataFrame({"column": [{"key": "this is a ° symbol"}]}),
        )
