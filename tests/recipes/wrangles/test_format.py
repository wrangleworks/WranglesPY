...
class TestFormatPrefix:
    ...
    def test_prefix_skip_empty_false(self):
        ...
        """
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert df["Tool Output"].tolist() == [
            "OSHA approved-Red White Blue Round Titanium Shield",
            'OSHA approved-300V 1/2" Drive Impact Wrench',
            "OSHA approved-",
            'OSHA approved-400 torque 1/2" Drive Impact Wrench',
            "OSHA approved-",
            "OSHA approved-Hard Hat 30in w/ Light",
        ]
...
class TestFormatPad:
    ...
    def test_pad_skip_empty_where_clause(self):
        ...
        """
        data = pd.DataFrame({"col1": ["7", "", "9"], "numbers": [3, 4, 5]})
        recipe = """  
        wrangles:  
            - format.pad:  
                input: col1  
    ...
    def test_pad_skip_empty_mixed_content(self):
        ...
        """
        data = pd.DataFrame({"col1": [7, "", "abc", "  ", 9]})
        recipe = """  
        wrangles:  
            - format.pad:  
                input: col1  
        ...
        df = wrangles.recipe.run(recipe, dataframe=data)
        assert (
            df.iloc[0]["out1"] == "00007"
            and df.iloc[1]["out1"] == ""
            and df.iloc[2]["out1"] == "00abc"
            and df.iloc[3]["out1"] == "  "
            and df.iloc[4]["out1"] == "00009"
        )
