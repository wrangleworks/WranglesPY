...
def test_classify_read_two_cols_wrgl(mocker):
    ...
    m1.return_value = {"Data": [["Hello", "Wrangles"], ["Hello", "Python"]]}
...
class TestTrainExtract:
    ...
    def test_extract_ai_write_name_and_model_id(self):
        ...
        with pytest.raises(
            ValueError,
            match="Name and model_id cannot both be provided, please use name to create a new model or model_id to update an existing model",
        ):
            wrangles.recipe.run(
                """
                write:
    ...
    def test_extract_ai_write_wrong_variant(self):
        ...
        """
        with pytest.raises(
            ValueError, match="The variant must be either 'pattern' or 'ai'"
        ):
            wrangles.recipe.run(
                """
                write:
    ...
    def test_extract_ai_write_model_and_variant(self):
        ...
        with pytest.raises(
            ValueError,
            match="It is not possible to set the variant of an existing model",
        ):
            wrangles.recipe.run(
                """
                write:
    ...
    def test_extract_ai_write_wrong_columns(self):
        ...
        with pytest.raises(
            ValueError,
            match="The columns Find, Description, Type, Default, Examples, Enum, Notes must be provided for train.extract",
        ):
            wrangles.recipe.run(
                """
                write:
...
class TestTrainLookup:
    ...
    def test_lookup_write_model_id_and_name(self):
        ...
        """
        with pytest.raises(
            ValueError, match="Name and model_id cannot both be provided"
        ):
            wrangles.recipe.run(
                """
                write:
    ...
    def test_lookup_write_no_model_id_or_name(self):
        ...
        """
        with pytest.raises(
            ValueError, match="Either a name or a model id must be provided"
        ):
            wrangles.recipe.run(
                """
                write:
