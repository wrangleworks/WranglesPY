...
def test_classify_error_3():
    ...
    assert (
        info.typename == "ValueError"
        and info.value.args[0]
        == f"Using extract model_id fce592c9-26f5-4fd7 in a classify function."
    )
...
# Extract
def test_address():
    result = wrangles.extract.address(
        "1100 Congress Ave, Austin, TX 78701, USA", "streets"
    )
...
def test_custom_first_element():
    result = wrangles.extract.custom(
        "test skf test timken", "fce592c9-26f5-4fd7", first_element=True
    )
...
def test_custom_first_element_list():
    result = wrangles.extract.custom(
        ["test skf test timken", "test timken test skf"],
        "fce592c9-26f5-4fd7",
        first_element=True,
    )
...
def test_extract_error_3():
    ...
    assert (
        info.typename == "ValueError"
        and info.value.args[0]
        == f"Using classify model_id a62c7480-500e-480c in an extract function."
    )


def test_extract_html_str():
    result = wrangles.extract.html(
        '<a href="https://www.wrangleworks.com/">Wrangle Works!</a>', dataType="text"
    )
...
def test_standardize_train_2():
    ...
    assert (
        info.typename == "ValueError"
        and info.value.args[0][:31] == "Training_data list must contain"
    )
...
def test_standardize_train_3():
    ...
    assert (
        info.typename == "ValueError"
        and info.value.args[0] == "A list is expected for training_data"
    )
...
def test_extract_train_1():
    data = [["Television", "TV"], ["Computer"]]
    config = {"training_data": data, "name": "test_extract"}
    with pytest.raises(ValueError) as info:
...
def test_extract_ai_model_id():
    ...
    assert (
        "Colors" in results
        and "Shapes" in results
        and isinstance(results["Colors"], list)
    )
...
def test_extract_ai_model_id_list():
    ...
    assert (
        ...
        and "Colors" in results[0]
        and "Shapes" in results[1]
        and isinstance(results[1]["Colors"], list)
    )
...
def test_extract_ai_output_schema_keys():
    ...
    results = wrangles.extract.ai(
        ...
        api_key=os.environ["OPENAI_API_KEY"],
        output={
            "Colors": {"type": "string", "description": "Any colors found in the input"}
        },
        retries=2,
    )

    assert "Colors" in results and isinstance(results["Colors"], str)
...
def test_extract_ai_output_schema():
    ...
    results = wrangles.extract.ai(
        "12 penguins",
        api_key=os.environ["OPENAI_API_KEY"],
        output={"type": "number", "description": "The number of penguins"},
        retries=2,
    )
...
def test_extract_ai_properties_list():
    ...
    result = wrangles.extract.ai(
        ...
        output={
            ...
            "items": {"type": "object", "properties": "unit,value"},
        },
        retries=2,
    )
...
def test_format_split_skip_empty_with_regex():
    ...
    """
    result = wrangles.format.split(
        ["hello,,world", "foo,,bar"], split_char="regex:,", skip_empty=True
    )
