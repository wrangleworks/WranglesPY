...
class train:
    ...
    def classify(training_data: list, name: str = None, model_id: str = None):
        ...
        if isinstance(training_data, list):
            check_index = [
                training_data.index(x)
                for x in training_data
                if len(x) != 3 or "" in x[:2]
            ]
            if len(check_index) != 0:
                raise ValueError(
                    f'Training_data list must contain a list of two non-empty elements, plus optional Notes. Check element(s) {check_index} in training_list.\nFormat:\nFirst element is "Example"\nSecond Element is "Category" -- \'\' is not valid.\n'
                    "Example:[['Rice', 'Grain', '']]"
                )
    ...
    def extract(
        training_data: list, name: str = None, model_id: str = None, variant: str = None
    ):
        """
        ...
        if isinstance(training_data, list) and variant in (None, "pattern"):
            ...
            if (
                len(check_index) != 0
            ):  # If an index does not have len() of 2 then raise error
                raise ValueError(
                    f"Training_data list must contain a list of two elements, plus optional Notes. Check element(s) {check_index} in training_list.\nFormat:\nFirst element is 'Entity to Find'\nSecond Element is 'Variation', If no variation, use ''\n"
        ...
        if isinstance(training_data, list) and variant == "extract-ai":
            ...
            if (
                len(check_index) != 0
            ):  # If an index does not have len() of 6 then raise error
                raise ValueError(
                    f"Training_data list must contain a list of six elements, plus optional Notes. Check element(s) {check_index} in training_list.\nFormat:\nFirst element is 'Find'\nSecond Element is 'Description', If no variation, use ''\n"
    ...
    def lookup(
        ...
        if isinstance(data, list):
            if len(data) < 2:
                raise ValueError(
                    "Lookup: The data must contain at least 1 header row and 1 row of contents"
                )
        ...
        if name is not None and settings.get("variant", "") not in [
            "key",
            "embedding",
            "fuzzy",
            "recipe",
        ...
        if not response.ok:
            raise RuntimeError(
                f"Training Lookup Failed. {response.status_code} : {response.text}"
            )
