...
class GeminiURLContextClient:
    ...
    Output Rules:
    - You MUST return a single valid object/dictionary at the root level (do NOT wrap the output in a list).
    - If info is missing for a string field, write "Not Found".
    - If info is missing for a list or dictionary, return it empty.
    - Generate a unique summary for the Description (do not recite).

    Required Structure:
    - Product: string (the official product name / title).
    - Brand: string (the product brand or manufacturer).
    - Category: string (the product category breadcrumb).
    - Description: string (summary text describing key features).
    - IDs: dictionary of strings (dynamic keys like "MPN", "UPC", "SKU").
        ...
        if not response.candidates:
            ...
            if output_format.lower() == "json":
                ...
                try:
                    ...
                    # UPDATED: Safety net. If the model still returns a list like [{}], unwrap it.
                    if isinstance(parsed_json, list):
                        parsed_json = parsed_json[0] if len(parsed_json) > 0 else {}
                        
                    result["extracted_content"] = parsed_json
