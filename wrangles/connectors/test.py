    ...
    if isinstance(data_type, str):
        if data_type == "<char>":
            ...
            try:
                ...
                random_list = _json.loads(
                    _re.search(r"<random\((.+)\)>", data_type).group(1)
                )
                return [
                    random_list[_random.randint(0, len(random_list) - 1)]
                    for _ in range(rows)
                ]
            except:
            ...
            try:
                num_range = _re.findall(r"\((\d+(\.\d+)?-\d+(\.\d+)?)\)", data_type)[0][
                    0
                ].split("-")
                try:
            ...
            return [
                round(
                    _random.uniform(float(num_range[0]), float(num_range[1])),
                    num_decimals,
                )
                for _ in range(rows)
            ]
            ...
            return [
                "".join(
                    _random.choice(_string.ascii_uppercase + _string.digits)
        ...
        # Get a random date from a range of dates
            ...
            if "to" in data_type:
                date_range_string = _re.findall(
                    r"\d+-\d+-\d+\sto\s\d+-\d+-\d+", data_type
                )[0]
