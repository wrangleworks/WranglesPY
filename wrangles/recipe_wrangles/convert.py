    ...
    for input_column, output_column in zip(input, output):
        if desired_case != "sentence":
            df[output_column] = df[input_column].apply(
                lambda x: _safe_str_transform(x, desired_case, warnings)
            )
            ...
            df[output_column] = df[input_column].apply(
                lambda x: _getSentenceCase(x, warnings)
            )
...
def data_type(
    ...
    **kwargs,
    ...
    if data_type == "datetime":
        ...
        dtype_dict = {"int": int, "float": float, "str": str, "bool": bool}

        # Raise an error if the datatype is not supported
        if data_type not in dtype_dict:
            raise TypeError(
                f"data_type {data_type} is not supported. Supported types are: {list(dtype_dict.keys()) + ['datetime']}"
            )
        else:
        ...
        for input_column, output_column in zip(input, output):
            ...
            df[output_column] = df.apply(
                lambda row: _safe_convert(
                    row[input_column], data_type, default, row.name
                ),
                axis=1,
            )
...
def from_json(
    ...
    **kwargs,
    ...
    # Loop through and apply for all columns
    for input_column, output_column in zip(input, output):
        df[output_column] = [_load_with_fallback(x) for x in df[input_column]]
