...
def dictionaries(
    df: _pd.DataFrame, input: list, output: str, skip_empty: bool = False
) -> _pd.DataFrame:
    """
    type: object
    ...
    if skip_empty:
        ...
        output_list = [
            {key: value for pair in row for key, value in pair.items()} for row in rows
        ]
...
def key_value_pairs(
    df: _pd.DataFrame, input: dict, output: str, skip_empty: bool = False
) -> _pd.DataFrame:
    """
    type: object
    ...
    results = [
        {
            row[key]: (
                True
                if val in cols_changed and row[val] == "True"
                else False if val in cols_changed and row[val] == "False" else row[val]
            )
            for key, val in pairs.items()
            if not skip_empty
            or (
                not _pd.isna(row[key])
                and not _pd.isna(row[val])
                and (not isinstance(row[key], str) or row[key].strip())
                and (not isinstance(row[val], str) or row[val].strip())
            )
        }
