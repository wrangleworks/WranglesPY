...
def pad(
    ...
    for input_column, output_column in zip(input, output):
        if skip_empty:
            ...
            df.loc[mask, output_column] = (
                df.loc[mask, input_column].astype(str).str.pad(pad_length, side, char)
            )
        else:
            df[output_column] = (
                df[input_column].astype(str).str.pad(pad_length, side, char)
            )
...
def remove_duplicates(
    ...
    # Loop through and apply for all columns
    for input_column, output_column in zip(input, output):
        df[output_column] = _format.remove_duplicates(
            df[input_column].values.tolist(), ignore_case
        )
...
def significant_figures(
    ...
    for input_column, output_column in zip(input, output):
        df[output_column] = _format.significant_figures(
            df[input_column].to_list(), significant_figures
        )
...
def trim(
    df: _pd.DataFrame, input: _Union[str, int, list], output: _Union[str, list] = None
) -> _pd.DataFrame:
    """
    type: object
    ...
    for input_column, output_column in zip(input, output):
        df[output_column] = df[input_column].apply(
            lambda x: _safe_str_transform(x, "strip", warnings)
        )
...
# Undocumented
def price_breaks(
    df: _pd.DataFrame, input: list, categoryLabel: str, valueLabel: str
) -> _pd.DataFrame:  # pragma: no cover
    """
    Rearrange price breaks
    """
    df = _pd.concat(
        [df, _format.price_breaks(df[input], categoryLabel, valueLabel)], axis=1
    )
    return df
