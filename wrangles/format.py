...
def flatten_lists(lst):
    return [
        item
        for sublist in lst
        for item in (flatten_lists(sublist) if isinstance(sublist, list) else [sublist])
    ]
    ...
    if skip_empty:
        return [
            (
                ...
                if isinstance(row, (list, _np.ndarray))
                else row
            )
...
def split(
    ...
    if element is not None:
        ...
        results = [_list_get(row, slicer, "") for row in results]
    ...
    if pad:
        ...
        if output_length is not None:
            ...
            # if more columns than number of splits, then add '' in extra columns
                ...
                results = [x + [""] * (output_length - len(x)) for x in results]
...
def remove_duplicates(input_list: list, ignore_case: bool = False) -> list:
    ...
    for row in input_list:
        ...
        if isinstance(row, list) and ignore_case:
            seen = set()
            results.append(
                [x for x in row if x.lower() not in seen and not seen.add(x.lower())]
            )
...
def tokenize(input, method="space", func=None, pattern=None):
    """
    ...
    def split_boundary_ignore_space(value):
        return [
            x for x in word_boundary_pattern.split(value) if x.replace(" ", "") != ""
        ]

    def split_boundary(value):
        return [x for x in word_boundary_pattern.split(value) if x != ""]
    ...
    if method == "space":
        ...
        raise ValueError(
            "Invalid method. Must be either 'space', 'boundary' or 'boundary_ignore_space'"
        )

    results = [
        (
            [item for sublist in [split_method(x) for x in item] for item in sublist]
