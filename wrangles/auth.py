...
def _refresh_access_token_from_refresh_token():
    ...
    try:
        ...
        response = _utils.request_retries(
            request_type="POST", url=url, **{"data": data}
        )
...
def _refresh_access_token():
    ...
    response = _utils.request_retries(
        request_type="POST", url=url, **{"data": payload, "headers": headers}
    )
...
def get_access_token():
    ...
    if _access_token == None or _access_token_expiry < _datetime.now():
        ...
        _access_token_expiry = _datetime.now() + _timedelta(
            0, response.json()["expires_in"] - 30
        )
