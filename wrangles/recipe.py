...
def _replace_templated_values(
    recipe_object: _typing.Any, variables: dict, ignore_unknown_variables: bool = False
) -> _typing.Any:
    """
    ...
    if isinstance(recipe_object, list):
        ...
        if "variables" in recipe_object and isinstance(
            recipe_object["variables"], dict
        ):
        ...
        new_recipe_object = {
            _replace_templated_values(
                key, temp_vars, any([ignore_unknown_variables, key in ["matrix"]])
            ): (
                _replace_templated_values(
                    val, temp_vars, any([ignore_unknown_variables, key in ["matrix"]])
                )
        ...
        if variable_pattern.fullmatch(new_recipe_object):
            ...
            if (
                ...
                try:
                    replacement_value = _yaml.load(
                        replacement_value, Loader=_YamlLoader
                    )
                except:
            ...
            new_recipe_object = _replace_templated_values(
                replacement_value, variables, ignore_unknown_variables
            )
        ...
        # Since this is within a string, the type is forced to also be a string
            ...
            for var in variable_pattern.findall(new_recipe_object):
                ...
                new_recipe_object = new_recipe_object.replace(
                    var, str(replacement_value)
                )
...
def _load_recipe(
    ...
    if not isinstance(recipe, str):
        try:
            ...
            recipe = _yaml.dump(
                recipe, sort_keys=False, Dumper=_YAMLDumper, allow_unicode=True
            )
        except:
    ...
    # If recipe matches xxxxxxxx-xxxx-xxxx, it's probably a model
        ...
        if model_functions:
            ...
            model_functions = {
                x.__name__: x for x in model_functions if _inspect.isfunction(x)
            }
        else:
            model_functions = {}
    ...
    if isinstance(functions, str):
        ...
        functions = [x for x in functions if _inspect.isfunction(x)]

    # Convert custom functions from a list to a dict using the name as a key
    if isinstance(functions, list):
        functions = {
            custom_function.__name__: custom_function for custom_function in functions
        }
    ...
    for action in recipe:
        if not isinstance(action, dict):
            if isinstance(action, str):
                ...
                raise ValueError(
                    "The run section of the recipe is not correctly structured"
                )

        for action_type, params in action.items():
            try:
                # If the action is conditional, check if it should be run
                if "if" in params and not _evaluate_conditional(
                    params["if"], variables
                ):
                    continue
                ...
                args = _add_special_parameters(
                    params, func, functions, variables, error, common_params
                )
                ...
                raise e.__class__(f"{action_type} - {e}").with_traceback(
                    e.__traceback__
                ) from None
    ...
    for read in recipe:
        if not isinstance(read, dict):
            if isinstance(read, str):
                ...
                raise ValueError(
                    "The read section of the recipe is not correctly structured"
                )

        for read_type, read_params in read.items():
            try:
                # If the action is conditional, check if it should be run
                if "if" in read_params and not _evaluate_conditional(
                    read_params["if"], variables
                ):
                    return None
                ...
                # Allow blended imports
                    ...
                    func = _get_nested_function(
                        read_type, _connectors, functions, "read"
                    )
    ...
    for step in wrangles_list:
        ...
        if not isinstance(step, dict):
            if isinstance(step, str):
                ...
                raise ValueError(
                    "The wrangles section of the recipe is not correctly structured"
                )
        ...
        for wrangle, params in step.items():
            try:
                if params is None:
                    params = {}
                _logging.info(
                    f": Wrangling :: {wrangle} :: {params.get('input', 'None')} >> {params.get('output', 'Dynamic')}"
                )
                ...
                if "input" in params and wrangle not in [
                    "math",
                    "maths",
                ...
                if "where" in params.keys():
                    if wrangle in _where_not_implemented:
                        raise NotImplementedError(
                            f"where parameter is not implemented for {wrangle}"
                        )
                ...
                if wrangle.split(".")[0] == "pandas":
                    ...
                    try:
                        df[params["output"]] = getattr(
                            df[params["input"]], wrangle.split(".")[1]
                        )(**params.get("parameters", {}))
                    except:
                        df = getattr(df, wrangle.split(".")[1])(
                            **params.get("parameters", {})
                        )
                    ...
                    if (
                        not isinstance(original_input, list)
                        and isinstance(params.get("input"), list)
                        and len(params.get("input", [])) == 1
                        and params["input"][0] == original_input
                    ):
                    ...
                    if "df" in fn_argspec.args:
                        ...
                        if not isinstance(df, _pandas.DataFrame):
                            raise RuntimeError(
                                f"Function {wrangle} did not return a dataframe"
                            )
                    ...
                    # Otherwise, do a row-wise apply
                        ...
                        for special_parameter in ["input", "output"]:
                            if (
                                special_parameter in params_temp
                                and special_parameter not in fn_argspec.args
                            ):
                        ...
                        if not fn_argspec.varkw:
                            ...
                            colDict = {
                                ...
                                if (
                                    " " in col
                                    and col.replace(" ", "_") in fn_argspec.args
                                )
                            }

                            df_temp = df_temp.rename(columns=colDict)
                            cols_renamed = [
                                col for col in cols_renamed if col in fn_argspec.args
                            ]
                        ...
                        if (
                            isinstance(params["output"], list)
                            and len(params["output"]) > 1
                        ):
                            result_type = "expand"
                        else:
                            result_type = "reduce"
                    ...
                    params = _add_special_parameters(
                        params, func, functions, variables, common_params=common_params
                    )
                    # Validate with a placeholder for df
                ...
                if (
                    "where" in original_params
                    and wrangle not in _where_overwrite_output
                ):
                    ...
                    if "output" in params.keys():
                        ...
                        if isinstance(params["output"], list):
                            ...
                            # Spread to a 1D list
                            output_columns = [
                                item for sublist in output_columns for item in sublist
                            ]
                    ...
                    elif list(df.columns) == list(
                        df_original.columns
                    ) and "input" in list(params.keys()):
                    ...
                    df = df.drop(
                        [col for col in df.columns if str(col).endswith("_x")], axis=1
                    )
                ...
                # Append name of wrangle to message and pass through exception
                raise e.__class__(f"{wrangle} - {e}").with_traceback(
                    e.__traceback__
                ) from None
...
def _filter_dataframe(
    ...
    **_,
    ...
    if where or order_by:
        ...
        df = df.loc[
            _recipe_wrangles.sql(
                df, sql, where_params, preserve_index=True, preserve_data_types=False
            ).index.to_list()
        ]
    ...
    if not_columns:
        ...
        remaining_columns = [
            column for column in list(df.columns) if column not in not_columns
        ]
...
def _write_data(
    df: _pandas.DataFrame, recipe: dict, functions: dict = {}, variables: dict = {}
) -> _pandas.DataFrame:
    """
    ...
    for export in recipe:
        if not isinstance(export, dict):
            if isinstance(export, str):
                ...
                raise ValueError(
                    "The write section of the recipe is not correctly structured"
                )
        ...
        for export_type, params in export.items():
            try:
                ...
                if export_type == "dataframe":
                    ...
                    func = _get_nested_function(
                        export_type, _connectors, functions, "write"
                    )
                ...
                # Append name of wrangle to message and pass through exception
                raise e.__class__(f"{export_type} - {e}").with_traceback(
                    e.__traceback__
                ) from None
...
def run(
    ...
    # Parse recipe
    recipe, functions = _load_recipe(recipe, variables, functions or {})

    with _futures.ThreadPoolExecutor(max_workers=1) as executor:
        try:
            future = executor.submit(
                _run_thread, recipe, variables, dataframe, functions
            )
