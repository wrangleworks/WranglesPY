...
def address(input: _Union[str, list], dataType: str, **kwargs) -> list:
    """
    ...
    params = {"responseFormat": "array", "dataType": dataType, **kwargs}
    batch_size = 10000
    ...
    **kwargs,
    ...
    if model_id is not None:
        if output_generic_key:
            raise ValueError(
                "Output must be set with keys when combining with a model_id"
            )
        ...
        try:
            ...
            messages = model_definition.get("settings", {}).get(
            ...
            model_definition = {
                x["find"]: {
                    ...
                    not in (
                        "",
                        None,
                    )  # ignore empty values but allow False and 0 as real possibilities
                }
            ...
            raise ValueError(
                f"Model definition for {model_id} is not correctly formatted"
            )

        output = {**model_definition, **(output or {})}
    ...
    def _standardize_schema(node):
        ...
        if (
            "examples" in node
            and not isinstance(node["examples"], list)
            and node["examples"] not in ("", None)
        ):
        ...
        if "properties" in node:
            ...
            if isinstance(node["properties"], list):
                node["properties"] = {v: {} for v in node["properties"]}

            # Clean up sub properties
            node["properties"] = {
                k: _standardize_schema(v) for k, v in node["properties"].items()
            }

        if (isinstance(node["type"], list) and "array" in node["type"]) or node[
            "type"
        ] == "array":
    ...
    output = {k: _standardize_schema(v) for k, v in output.items()}
    ...
    kwargs = {**default_settings.get(model, {}), **kwargs}
...
def attributes(
    ...
    **kwargs,
    ...
    params = {"responseFormat": "array", "responseContent": responseContent, **kwargs}
    if type:
...
def codes(input: _Union[str, list], first_element: bool = False, **kwargs) -> list:
    """
...
def custom(
    ...
    **kwargs,
    ...
    if isinstance(input, str):
        ...
        raise TypeError(
            "Invalid input data provided. The input must be either a string or a list of strings."
        )
    ...
    if [len(x) for x in model_id.split("-")] != [8, 4, 4]:
        raise ValueError(
            "Incorrect or missing values in model_id. Check format is XXXXXXXX-XXXX-XXXX"
        )
    ...
    if isinstance(results, list):
        ...
        if use_labels and first_element:
            results = [
                {k: v[0] for (k, v) in zip(objs.keys(), objs.values())}
                for objs in results
            ]
    else:
        raise ValueError(
            f"API Response did not return an expected format for model {model_id}"
        )
...
def html(input: _Union[str, list], dataType: str, **kwargs) -> list:
    """
...
def properties(
    ...
    **kwargs,
    ...
    if return_data_type == "string":
        results = [", ".join(x) if x != [] else "" for x in results]
        ...
        for remove in _remove:
            ...
            if tokenize_to_remove == True:
                ...
                for subtoken in token_remove:
                    subtoken = _re.escape(
                        subtoken
                    )  # escape the special characters just in case
...
def brackets(
    input: str, find: list = _Union[str, list], include_brackets: bool = False
) -> list:
    """
