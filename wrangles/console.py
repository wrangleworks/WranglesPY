...
def recipe():
    ...
    parser = _argparse.ArgumentParser(
        prog="wrangles.recipe", description="Run a Wrangles Recipe"
    )

    parser.add_argument("recipe", type=str, help="The filename of the recipe")
    parser.add_argument(
        "--functions", "-f", type=str, help="A file of custom functions"
    )
    parser.add_argument(
        "--variables", "-v", type=open, help="A file containing custom variables"
    )
    parser.add_argument(
        "--varDict",
        type=str,
    ...
    # Run the recipe
    _recipe.run(
        args.recipe, functions=args.functions, variables=variables, timeout=args.timeout
    )
