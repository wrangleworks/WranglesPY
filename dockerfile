...
RUN cd /opt/venv/lib/python3.10/site-packages/botocore/data && cp -r s3 _retry.json endpoints.json sdk-default-configuration.json /tmp/
RUN rm -r /opt/venv/lib/python3.10/site-packages/botocore/data/*
RUN cp -r /tmp/s3 /tmp/_retry.json /tmp/endpoints.json /tmp/sdk-default-configuration.json /opt/venv/lib/python3.10/site-packages/botocore/data
